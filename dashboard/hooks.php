        ...
        <div class="invoice_table">
            <div class="invoice_data">
                ...
                if (
                    ...
                    || $invoice_saved == esc_html__('Reservation fee', 'wprentals')
                ) { ?>
                    ...
                    if ($booking_array['price_per_guest_from_one'] == 1) { ?>
                        ...
                        <span class="date_duration show_invoice_price_per_night">
                            ...
                            if ($booking_array['cover_weekend']) {
                                echo ', '
                                     . esc_html__('has weekend price of', 'wprentals')
                                     . ' '
                                     . $price_per_weekeend_show;
                            }
                            ?>
                    ...
                    if ($detail[1] != 0) {
                        ...
                        <div class="invoice_row invoice_content">
                            ...
                            <span class="inv_exp">
                                <?php
                                if (
                                    trim($detail[0]) == esc_html__('Security Depozit', 'wprentals')
                                    || trim($detail[0]) == esc_html__('Security Deposit', 'wprentals')
                                ) {
...
function wpestate_ajax_update_listing_description(): void
{
    ...
    main_check_before_update_listing();
    ...
    $prop_category        = isset($_POST['category']) ? intval($_POST['category']) : '';
    $prop_action_category = isset($_POST['action_category']) ? wp_kses($_POST['action_category'], $allowed_html) : 0;
    $property_city        = isset($_POST['city']) ? wp_kses($_POST['city'], $allowed_html) : 0;
    $property_country     = isset($_POST['country']) ? wp_kses($_POST['country'], $allowed_html) : '';
    $property_area        = isset($_POST['area']) ? wp_kses($_POST['area'], $allowed_html) : 0;
    $property_affiliate   = isset($_POST['aff_link']) ? esc_url($_POST['aff_link']) : '';
    $private_notes        = isset($_POST['private_notes']) ? esc_html($_POST['private_notes']) : '';
    $overload_guest       = isset($_POST['overload_guest']) ? floatval($_POST['overload_guest']) : 0;
    $max_extra_guest_no   = isset($_POST['max_extra_guest_no']) ? intval($_POST['max_extra_guest_no']) : 0;

    $property_admin_area = isset($_POST['property_admin_area']) ? wp_kses(
        $_POST['property_admin_area'],
        $allowed_html
    ) : '';
    ...
    if ($prop_category == -1) {
        ...
        $prop_category_term = get_term($prop_category, 'property_category');
    }

    if ($prop_action_category == -1) {
        wp_delete_object_term_relationships($edit_id, 'property_action_category');
    } else {
        $prop_action_category_term = get_term($prop_action_category, 'property_action_category');
    }
    ...
    if (isset ($prop_action_category_term->name)) {
        wp_set_object_terms($post_id, $prop_action_category_term->name, 'property_action_category');
    }

    if (isset($prop_category_term->name)) {
        wp_set_object_terms($post_id, $prop_category_term->name, 'property_category');
    }
...
function wpestate_ajax_update_listing_images(): void
{
    ...
    $new_thumb        = 0;
    $current_thumb    = get_post_thumbnail_id($edit_id);
    ...
    if (function_exists('icl_translate')) {
        ...
        foreach ($post_attachments as $attachment) {
            if ( ! in_array($attachment->ID, $attach_array)) {
                wp_delete_post($attachment->ID);
                if ($current_thumb == $attachment->ID) {
                    $new_thumb = 1;
                }
            }
        }
...
/**
 * Main checks before update listing from dashboard
 *
 * @return void
 */
