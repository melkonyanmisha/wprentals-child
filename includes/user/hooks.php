...
function wpestate_ajax_register_form(): void
{
    ...
    $user_name  = isset($_POST['user_login_register']) ? trim(sanitize_text_field($_POST['user_login_register'])) : '';
    $user_email = isset($_POST['user_email_register']) ? trim(sanitize_text_field($_POST['user_email_register'])) : '';
    ...
    if (filter_var($user_email, FILTER_VALIDATE_EMAIL) === false || ! checkdnsrr($email_domain)) {
        wp_die(
            json_encode([
                'register' => false,
                'message'  => esc_html__('Wrong email.', 'wprentals')
            ])
        );
    }
    ...
    if (wprentals_get_option('wp_estate_use_captcha', '') == 'yes') {
        $confirm_error_msg = esc_html__('Please confirm you are not a robot.', 'wprentals');

        if (empty($_POST['capthca'])) {
            wp_die(
                json_encode(['register' => false, 'message' => $confirm_error_msg])
            );
        }
    ...
    if ($enable_user_pass_status == 'yes') {
        ...
        if ($user_pass !== $user_pass_retype) {
            wp_die(
                json_encode([
                    'register' => false,
                    'message'  => esc_html__('Passwords don\'t match.', 'wprentals')
                ])
            );
        }
    }
